app.controller('taskOverFlowController', function($scope, $http, $state, $rootScope,cloudService,valueService) {
    $scope.myVar = false;
    ...
    function getDebriefStagesInfo(){
        ...
        $http.get("app/json/notes.json").then(function(response) {
            console.log('*****Notes Response*****');
            console.log(response.data.Notes);


            angular.forEach(response.data.Notes,function(key,value)
            {
                var noteObj={}
                if(key.Task_Number==$scope.taskId)
                {
...
$http.get("app/json/contacts.json").then(function(response) {
    console.log('***Contacts Response***');
  var contactArray=[]
    angular.forEach(response.data.Contacts,function(key,value)
    {
        var contactObj={}
        if(key.Task_Number==$scope.taskId)
        {
...
$http.get("app/json/installBaseCloud.json").then(function(response) {
    console.log('***Install Base Response');
